...
def make_packet_List(content, filename):
    ...
    while len(content) != 0:
        data = content[0:64]
        csum = checksum(data, 'data_pkt', number=num, status=False)
        data = jsonify(data, pkt_type='data_pkt', number=num, status=False, csum=csum)
    ...
    csum = checksum(data, 'close_pkt', number=num, status=False)
    data = jsonify(data, pkt_type='close_pkt', number=num, status=False, csum=csum)
    packet_list.append(data)
    return packet_list
...
def createDupAck(data_num):
    data = 'DupAck Sent'
    data = bytes(data, 'utf-8')
    csum = checksum(data, 'DupAck_pkt', number=data_num, status=True)
    ackpkt = jsonify(data, 'DupAck_pkt', number=data_num, status=True, csum=csum)
    return ackpkt
